...
class LIPM_motion_generator:
    ...
    def gaitGeneration(self, period=2, dt=0.01, footStep=2, kDSP=0.4419, alter_z=0):
        self.period = period
        self.dt = dt
        ...
        # zcSinusoidR = -0.0012 * np.cos(2 * np.pi * t / self.period) - self.alter_z * Rsinu
        # zcSinusoidL = -0.0012 * np.cos(2 * np.pi * t / self.period) - self.alter_z * Lsinu

        t_values = np.linspace(0, 2*np.pi, 35)  # 时间范围
        Rsinu = np.concatenate([np.zeros(14),sinusoidal_wave(t_values),np.ones(91)])
        Lsinu = np.concatenate([np.zeros(84),sinusoidal_wave(t_values),np.ones(21)])
        zcSinusoidR = -0.0012 * np.cos(2 * np.pi * t / self.period) - self.alter_z * Rsinu
        zcSinusoidL = -0.0012 * np.cos(2 * np.pi * t / self.period) - self.alter_z * Lsinu
        ...
        # mR = -0.03/(70*(1-self.kDSP))
        # for i in(range(85,125)):
        #     zPR[0][i]=mR*(i-85)+zPR[0][85]
