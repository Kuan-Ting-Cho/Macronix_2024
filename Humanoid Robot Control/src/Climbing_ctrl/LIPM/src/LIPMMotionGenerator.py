...
class LIPM_motion_generator:
    ...
    def gaitGeneration(self, period=2, dt=0.01, footStep=2, kDSP=0.4419, alter_zCoM=0):
        self.period = period
        self.dt = dt
        self.footStep = footStep
        self.kDSP = kDSP
        self.alter_zCoM = alter_zCoM
        ...
        zcSinusoidR = -0.0012 * np.cos(2 * np.pi * t / self.period) - alter_zCoM * Rsinu
        zcSinusoidL = -0.0012 * np.cos(2 * np.pi * t / self.period) - alter_zCoM * Lsinu

        # footZ[0, i] = zAmp / 2 * (1 - np.cos(2 * np.pi * (t - t1) / tSSP)) * (
        #     np.heaviside(t - t1 - dt, 0.5) - np.heaviside(t - t2, 0.5)) +  0.05 * Rsinu[i]
        ...
        # plt.plot(zPR[0] + self.footHeight, label='PRz', color='blue')
        # plt.plot(zPL[0] + self.footHeight, label='PLz', color='red')
