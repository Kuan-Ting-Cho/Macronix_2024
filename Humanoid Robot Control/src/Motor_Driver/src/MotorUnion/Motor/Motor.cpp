...
Motor::Motor(
	...
	: motor(),
	  ...
	  ADDR_PRESENT_CURRENT(addr_present_current),
	  ...
	  LEN_PRESENT_CURRENT(len_present_current),
...
const int &Motor::GetMotorID() const { return Motor_ID; }
...
void Motor::AddParamPresentCurrent()
{
	groupBulkRead->addParam(Motor_ID, ADDR_PRESENT_CURRENT, LEN_PRESENT_CURRENT);
}
...
void Motor::ReadPresentCurrent()
{
	int16_t data = groupBulkRead->getData(Motor_ID, ADDR_PRESENT_CURRENT, LEN_PRESENT_CURRENT);
	// Motor_Present_Torque = data * 100.f / Max_Torque_Limit;
	Motor_Present_Current = data;
}
...
// }
void Motor::WriteMode() // NOT WORKING!!!
{
	uint8_t param_mode[1];
	param_mode[0] = Motor_Operating_Mode;
	groupBulkWrite->addParam(Motor_ID, ADDR_OPERATING_MODE, 1, param_mode);
}
...
void Motor::WriteCurrent()
{
	uint8_t param_goal_current[LEN_GOAL_CURRENT];
	param_goal_current[0] = DXL_LOBYTE(DXL_LOWORD(Motor_Current));
	param_goal_current[1] = DXL_HIBYTE(DXL_LOWORD(Motor_Current));
