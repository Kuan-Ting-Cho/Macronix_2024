...
def Data_preprocess(motion,footstep,isflat):
    ...
    # for i in range(1,len(motion)):
    #     # if i >= 50 and i<100:
    #     #     if motion[i][9]>motion[50][9]:
    #     #         motion[i][9]=motion[50][9]
    #     if i < 30 or i > 130:
    #         if motion[i][9]<motion[0][9]:
    #             motion[i][9]=motion[0][9]
    ...
    for i in range(30):
        motion= np.append(motion,np.array([motion[-1]]),axis=0)
    motion=np.transpose(motion)
    for i in range(len(motion)):
