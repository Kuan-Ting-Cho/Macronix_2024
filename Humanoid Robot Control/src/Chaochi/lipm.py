...
class Forward2:
    ...
    def output_motion(self,desired,footstep):
        ...
        if self.motionGen == True: # Motion 檔 
            motionFilePath = 'Chaochi/src/' + self.fileName
            LIPM_motion.writeFile(outputData, motionFilePath, initPose, scale, dir)

        # temp return
        motion = pd.read_csv('Chaochi/src/F2_200.csv',header=None,index_col=None)
        motion=Data_preprocess(motion,footstep)
        # pd.DataFrame(motion).to_csv('Chaochi/src/F2_200.csv',header=None,index=False)
        return CoM,Rd_end,Ld_end