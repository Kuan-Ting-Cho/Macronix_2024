...
def ModifiableFootGeneration(Amp, T, dt, tDSP): #p.20
    ...
    Args:
        Amp (list of list): [leg length, shift, step height]
        T (int): self.period, usually 1
        dt (float): 0.01
        tDSP (float): kDSP*T = 0.4419*1

    Returns:
        _type_: _description_
    """
    N = Amp.shape[1] # N is number of steps
    ...
    for t_temp in t:
        ...
        if t <= t1+(t2-t1)/3 and t >= t1:
            footZ[0, i] = zAmp / 2 * (1 - np.cos(1.5*2 * np.pi * (t - t1) / tSSP)) * (
                np.heaviside(t - t1 - dt, 0.5) - np.heaviside(t - t2, 0.5))#2.41
            # print(footZ[0, i])
        elif t > t1+(t2-t1)/3 and t <= t2:
            footZ[0, i] = zAmp / 2 * (1 - np.cos(2/3*2 * np.pi * (t - t1+(t2-t1)/3+ dt) / tSSP)) * (
                np.heaviside(t - t1+(t2-t1)/3 - dt, 0.5) - np.heaviside(t - t2, 0.5))#2.41
        
        # footZ[0, i] = zAmp / 2 * (1 - np.cos(2 * np.pi * (t - t1) / tSSP)) * (
        #     np.heaviside(t - t1 - dt, 0.5) - np.heaviside(t - t2, 0.5)) + 0.05 * np.heaviside(t - t2, 1) + \
        #     + 0.05 * (xBias + xAmp / 2 / np.pi * (2 * np.pi*(t - t1) / tSSP - np.sin(2 * np.pi*(t - t1) / tSSP)) * (np.heaviside(t - t1 - dt, 0.5) - np.heaviside(t - t2, 0.5))\
        #     + xAmp * np.heaviside(t - t2, 0.5))      
...
def OutputMotion(PR, PL, hip1, hip2, leanAngleR, leanAngleL, yawAngleR,
                 yawAngleL, pedalRollAngleR, pedalRollAngleL, legLinkLength,turn, rightFirst, T, dt):
    N = PR.shape[1]
    thetaR = []
    thetaL = []
    for i in range(N):
