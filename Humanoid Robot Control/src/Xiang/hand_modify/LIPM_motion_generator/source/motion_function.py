...
from source.Forward_Kinematics import all_transfomation_matrix

# Pykin tools #
import numpy as np
from pykin.robots.bimanual import Bimanual
from pykin.kinematics.transform import Transform
import pykin.utils.transform_utils as T_u
...
def OutputMotion(initR, initL, Rup, T, sampleT, dt, hip, Legs, thetaR_length,\
    ...
    samplek = sampleT / dt
    d2 = 6 / 1000
    a=np.array([105, 102, 357.95, 366.42, 29, 111.75])/1000
    
    ####################################
    thetaR_leg = []
    thetaL_leg = []
    file_path = 'urdf/test27/test27.urdf'

    robot = Bimanual(file_path, Transform(rot=[0.0, 0.0, 0.0], pos=[0, 0, 0]))
    robot.setup_link_name("base", "L_ankle_roll")
    robot.setup_link_name("base", "R_ankle_roll")
    init_thetas = np.random.rand(6)
    ...
    for i in range(1, thetaR_length+1):
        ...
        ####################################
        # target_R = T_u.get_quaternion_from_matrix(np.array(R_R))
        # target_R = np.append(P_R, target_R)
        # target_L = T_u.get_quaternion_from_matrix(np.array(R_L))
        # target_L = np.append(P_L, target_L)


        # target_pose = { "right": target_R, 
        #                 "left" : target_L}
        ...
        #     # print("result ", ik_result)
        #     head_thetas =  np.zeros(1)
        #     thetas_LM = np.concatenate((head_thetas, ik_result["right"], ik_result["left"]))
        #     result_fk_LM = robot.forward_kin(thetas_LM)
        #     robot.set_transform(thetas_LM)

        #     _, ax = p_utils.init_3d_figure("LM IK Result", visible_axis=True)
        ...
        thetaR_leg.append(InvK(d2, a[1:], R_R, P_R))
        thetaL_leg.append(InvK(d2, a[1:], R_L, P_L))
        # thetaR_leg.append(ik_result["right"])
        # thetaL_leg.append(ik_result["left"])
    ...
    # motor rotation +/-
    Motor_Dir_R = [ 1,  -1,  1, 1, 1, -1] #1108
    Motor_Dir_L = [ 1,  -1,  1, 1, 1, -1]

    # # motor rotation +/-
    # Motor_Dir_R = [ 1,  1,  1,  -1, 1,  1] #1105
    # Motor_Dir_L = [-1, -1, -1, -1, 1, -1]
    ...
    for i in range(thetaR_length):
        ...
        for j in range(6):
            # For Sim   
            ScaledthetaR_leg_test[j][i] = (MotorOffsetR[j] + thetaR_leg[i][j] * Motor_Dir_R[j] * MScaleR_Leg[j])
            ScaledthetaL_leg_test[j][i] = (MotorOffsetL[j] + thetaL_leg[i][j] * Motor_Dir_L[j] * MScaleL_Leg[j])
            # For Real
